...
class Region{
	...
	protected static $table_name="region2";
	protected static $db_fields = array('region_id', 'region_name', 'depot_id');
	public $region_id;
	public $region_name;
	public $depot_id;
	//public $depot_code;
    ...
    $result_array= self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE empid='$empid' LIMIT 1");
  	...
  	public static function find_by_region_name($region_name=""){
	$result_array= self::find_by_sql("SELECT * FROM ".self::$table_name." WHERE region_name='$region_name' LIMIT 1");
		return !empty($result_array) ? array_shift($result_array) : false;
  	}
