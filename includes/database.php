...
class MySQLDatabase {
    ...
    public function open_connection() {
        //$this->connection = mysqli_connect("50.62.209.79", "approvalsystem", "Akshay1234");
        $this->connection = mysqli_connect("localhost", "root","");
        if (!$this->connection) {
            die("Database connection failed: " . mysql_error());
        } else {
            $db_select = mysqli_select_db($this->connection, "approvalsystem");
            if (!$db_select) {
    ...
    public function result($result_set) {
        return mysqli_num_rows($result_set);
    }
